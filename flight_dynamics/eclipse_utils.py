...
def brute_force_eclipse_angles(kep_state: np.ndarray,
    ...
    ta_grid = np.linspace(-np.pi, np.pi, 200)
...
def compute_eclipse_angles(kep_state: np.ndarray,
    ...
    Order of magnitude faster than `brute_force_eclipse_angles` but lower resolution
    because it relies on a simplified cylindrical shadow assumption.

    Not tested in this codebase, but Lundberg (1995) suggests that variations from 
    the penumbra-umbra corrections from the cylindrical shadow assumption are about 
    one minute.
    ...
    #DEBUG
    r_sun_earth = np.array([-r_sun_earth_norm,0,0])
    ...
    # shadow_fn = lambda x: (
    #     (Constants.R_EARTH**2)*((1+ecc*x)**2) +
    #     (slr**2)*((beta1*x + beta2*np.sqrt(1-x**2))**2) - 
    #     slr**2
    # )
    ...
    shadow_roots = np.roots(coeffs)

    # # Domain for x = cos(true anomaly)
    ...
    for cos_ta in shadow_roots:
        ...
        if np.isreal(cos_ta) and -1 <= cos_ta.real <= 1:
            # cos(sigma) < 0 -> shadow occurs
                ...
                for ta in ta_candidates:     
                    if beta1*np.cos(ta) + beta2*np.sin(ta) < 0:
                        # actual_shadow_roots.append(cos_ta)
                        ...
                        if S_dot > 0:
                            ...
                            raise ValueError("S_dot is zero.")
    
    if ta_entry is not None and ta_exit is not None:
        return ta_entry, ta_exit
    if ta_entry is None and ta_exit is None:
        return 0.0,0.0
    else:
        raise ValueError("Both entry and exit angle must be defined or both must be not defined.")
