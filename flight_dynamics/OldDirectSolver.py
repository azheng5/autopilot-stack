...
class OldDirectSolver:
    ...
    def terminal_constraints(self,
        ...
        final_ecc = final_x_bar[1]
        print(f"{final_x_bar[0]},{final_x_bar[1]}")

        constraint_dict = {
            "eq_constraint": np.array([final_sma - sma_grid[-1]]),
            "ineq_constraint": np.array([final_ecc - 1e-6]),
        }
    ...
    def orbit_averaged_derivative(self, 
        ...
        E_en_shifted = np.pi # TODO actually compute it for now just trying to see if solver works for eclipseless case tho
        E_ex_shifted = -np.pi
        E_shifted_grid = np.linspace(E_ex_shifted, E_en_shifted, 100)
        ...
        integrand_grid = np.zeros((len(E_shifted_grid),6))
        for ind in range(len(E_shifted_grid)):
            E_i = E_shifted_grid[ind]
        ...
        integral_value = np.trapezoid(integrand_grid, E_shifted_grid, axis=0)
        ...
        x_bar_j2_dot = np.array([0,0,0,0,0,0])

        total_x_bar_dot = x_bar_dot + x_bar_j2_dot

        return total_x_bar_dot
    ...
    def orbit_integrand(self, 
        ...
        Arguments:
            - E_shifted: Eccentric anomaly (rad)
        ...
        ecc = x_bar[1]
        inc = np.mod(x_bar[2], 2*np.pi)
        raan = np.mod(x_bar[3], 2*np.pi)
        aop = np.mod(x_bar[4], 2*np.pi)
        m = x_bar[5]
        ...
        mean_motion = astro_utils.compute_mean_motion(sma)
        ta_shifted = astro_utils.eccentric2true(E_shifted, ecc)
        r = astro_utils.compute_radius(sma, ecc, ta_shifted)
        v = astro_utils.vis_viva(sma, r)

        # Compute thrust acceleration components
        alpha = self.compute_thrust_angle(sma,ecc,r,v,ta_shifted,
                                          lambda_a,lambda_e,lambda_i)
        beta = 0 #TODO currently yaw control not supported
        A_mag = 2.943e-4 #TODO hardcoded rn, fix it
        A_mag_kN = A_mag * 1e-3
        a_thrust = self.compute_nth_thrust_components(A_mag_kN, alpha, beta)
        
        # Construct full state
        full_state = np.array([sma,ecc,inc,raan,aop,ta_shifted,m])
    ...
    def keplerian_diff_eq(self,
        ...
        sma = full_state[0]
        ecc = full_state[1]
        inc = np.mod(full_state[2], 2*np.pi)
        # raan = full_state[3]
        aop = np.mod(full_state[4], 2*np.pi)
        ta_shifted = full_state[5]
        # m = full_state[6]
        ...
        ecc_dot = (1/v) * ( 2*(ecc + np.cos(ta_shifted))*a_t + (r/sma)*a_n*np.sin(ta_shifted) )
        inc_dot = (r/h) * a_h * np.cos(aop + ta_shifted)

        # RAAN not defined at zero inclination
        if inc != 0:
            raan_dot = (r/(h*np.sin(inc))) * a_h * np.sin(aop + ta_shifted)
        else:
            raan_dot = 0.0

        # AOP, TA  not defined at zero eccentricity
        if ecc > 0:
            aop_dot = (1/(ecc*v)) * ( 2*a_t*np.sin(ta_shifted) - (2*ecc+(r/sma)*np.cos(ta_shifted))*a_n ) - (r/(h*np.sin(inc))) * a_h * np.sin(aop + ta_shifted) * np.cos(inc)
            ta_dot = (h/(r**2)) - (1/(ecc*v)) * ( 2*a_t*np.sin(ta_shifted) - (2*ecc+(r/sma)*np.cos(ta_shifted))*a_n )
        else:
            aop_dot = 0.0
            ta_dot = 0.0
